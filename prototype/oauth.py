...
twitter_blueprint= make_twitter_blueprint(api_key= CONSUMER_KEY1, api_secret= CONSUMER_SECRET1)
app.register_blueprint(twitter_blueprint, url_prefix='/authorized')

#FOR DATABASE:#######################################
app.config['SQLALCHEMY_DATABASE_URI']= 'PATH to sqlite database'
db = SQLAlchemy(app)
...
def twitter_login():
	...
	account_info= twitter.get('account/settings.json')       #SOME INDENTATION ERROR
